...
ALLOWED_CONCEPT_CATS = ['DISO', 'T004', 'T005', 'T007', 'T204', 'T032', 'T055', 'T061', 'T043','T116', 'T129']
...
if __name__ == '__main__':
    ...
    MMQUERY_CONDITION_INDEX = 3
    MMQUERY_CONDITION_REQUEST = 4
    MMQUERY_CONDITION_CONCEPT = 5
    MMQUERY_CONDITION_CUI = 6
    MMQUERY_CONDITION_CATEGORY = 7
    MMQUERY_CONDITION_ALL_CONCEPTS = 8
    ...
    if not MMQUERY_MM_MEMOIZE:
        ...
        # Read manually assigned concepts .tsv file for conditions
        conditions_remap_file = "utils/CT_conditions_manual_remaps.tsv"
        with open(conditions_remap_file, 'r') as my_file:
            ...
            for idx, row in enumerate(reader):
                ...
                if idx == 0:
                    ...
                    remapped_request = row[fieldIndex["Condition_request"]]
                    remapped_cui = row[fieldIndex["Condition_Cui"]]
                    remapped_concept = row[fieldIndex["Condition_Concept"]]
                    remapped_category = row[fieldIndex["Condition_CAT"]]
    ...
    for row in csv_in_rows:
        ...
        term_display_nohyp = map_global_terms(clean_entity_string(substract_hyphens(output_rows[-1][MMQUERY_CONDITION_INDEX].lower())))
        ...
        if request_string in premm_dictionary:
            ...
            output_rows[-1][MMQUERY_CONDITION_REQUEST] = request_string
            output_rows[-1][MMQUERY_CONDITION_CONCEPT] = preferred
            output_rows[-1][MMQUERY_CONDITION_CUI] = cui
            output_rows[-1][MMQUERY_CONDITION_CATEGORY] = semtypes
            output_rows[-1][MMQUERY_CONDITION_ALL_CONCEPTS] = all_concepts.replace(MMQUERY_CSV_DELIMITER, ',')
        ...
        # otherwise we need to query MetaMap, but we will launch queries in batches for speed
            ...
            # avoid querying MetaMap for empty strings or punctuation chars, and consider them completed
                ...
                output_rows[-1][MMQUERY_CONDITION_REQUEST] = request_string
                output_rows[-1][MMQUERY_CONDITION_CONCEPT] = MMQUERY_CSV_DEFAULT_FIELD
                output_rows[-1][MMQUERY_CONDITION_CUI] = MMQUERY_CSV_DEFAULT_FIELD
                output_rows[-1][MMQUERY_CONDITION_CATEGORY] = MMQUERY_CSV_DEFAULT_FIELD
                output_rows[-1][MMQUERY_CONDITION_ALL_CONCEPTS] = MMQUERY_CSV_DEFAULT_FIELD
                output_rows_dict_mapped[-1].append(True)
        ...
        if len(batch_request_strings) >= REQUEST_SIZE or (row_index + 1) == row_count:
            ...
            for i in reversed(range(len(metamap_results_primary))):
                ...
                output_rows[out_rows_index][MMQUERY_CONDITION_REQUEST] = r_string
                output_rows[out_rows_index][MMQUERY_CONDITION_CONCEPT] = preferred
                output_rows[out_rows_index][MMQUERY_CONDITION_CUI] = cui
                output_rows[out_rows_index][MMQUERY_CONDITION_CATEGORY] = semtypes
                output_rows[out_rows_index][MMQUERY_CONDITION_ALL_CONCEPTS] = all_concepts.replace(MMQUERY_CSV_DELIMITER, ',')
    ...
    if len(failed_requests_mappings) > 0:
        ...
        for out_index in range(len(output_rows)):
            if out_index == 0:
                ...
                if output_rows[out_index][MMQUERY_CONDITION_REQUEST] in failed_requests_mappings:
                    count_updates += 1
                    [score, cui, preferred, semtypes, matched_candidate] = failed_requests_mappings[output_rows[out_index][MMQUERY_CONDITION_REQUEST]][0]
                    all_concepts = failed_requests_mappings[output_rows[out_index][MMQUERY_CONDITION_REQUEST]][1]
                    output_rows[out_index][MMQUERY_CONDITION_CONCEPT] = preferred
                    output_rows[out_index][MMQUERY_CONDITION_CUI] = cui
                    output_rows[out_index][MMQUERY_CONDITION_CATEGORY] = semtypes
                    output_rows[out_index][MMQUERY_CONDITION_ALL_CONCEPTS] = all_concepts.replace(MMQUERY_CSV_DELIMITER, ',')
                    premm_dictionary[output_rows[out_index][MMQUERY_CONDITION_REQUEST]] = (failed_requests_mappings[output_rows[out_index][MMQUERY_CONDITION_REQUEST]][0] + [1], all_concepts)
