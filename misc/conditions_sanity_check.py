...
with open(file_no_cache, 'r') as my_file:
    ...
    for idx, row in enumerate(reader):
        ...
        if idx == 0:
            ...
            if not idx % 100000:
                print(idx)
            request = row[field_index["cond_mm_request"]]
            concept = row[field_index["condition_concept"]]
            cui = row[field_index["condition_cui"]]
            all_mm = row[field_index["condition_all_mm"]]
            cuis.add(cui)
            if request not in no_cache_mappings:
                ...
                if cui != no_cache_mappings[request][1]:
                    print(no_cache_mappings[request][-1])
